...
class SensorDataService {
  ...
  static async getSensorDataList(
    ...
    try {
      ...
      if (crudParams.sort_field) url += `&sort_field=${crudParams.sort_field}`;
      if (crudParams.sort_order) url += `&sort_order=${crudParams.sort_order}`;
  ...
  static async getSensorDataById(sensorId) {
    try {
      const response = await fetch(`${API_BASE_URL}/sensor-data/${sensorId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      return await response.json();
